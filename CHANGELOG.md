...
**启动更快、多 Agent 规划、质量与设置体验升级**

### 新功能

- 有限多 Agent：复杂任务先自动拆成可核对步骤（默认开启，最多 3 路并发），写文件/Shell/微信仍由主 Agent 审批
- 插件清单统一为 friday-plugin.json，旧格式启动时自动迁移

### 改进

- 启动加速：工具模块按需加载，减少冷启动 import 耗时
- 设置页拆分主题与视觉/生图配置；保存后状态栏立即刷新，无需切页
- 发版依赖锁定与校验（requirements-lock），CI 测试覆盖率门槛 55%
- 文档与架构说明与当前代码对齐

### 修复

- 修复测试版（Friday 测试版快捷方式）无法打开：启动时 guard_startup_after_update 未导入
...
- 「手动下载」改为安装程序 Friday-Setup，避免误解压 Update.zip 后闪退
- 安装/更新完成后仅短暂尝试置顶窗口，切到其他应用后不再反复弹出

### 修复

- 修复 Gitee 一键更新 SHA256 校验失败（Release 清单与安装包不一致）
- 修复一键更新「下载解压后关闭却未升级」：更新脚本脱离进程树，避免被旧进程连带终止
- 修复 apply-update.ps1 在中文 Windows 上因编码导致 PowerShell 解析失败、静默不安装
- 一键更新替换失败时会弹窗提示；下次启动也会提醒上次更新未完成

---

## 1.4.9（2026-06-18）

**PPT 技能内置、更新不丢设置、发版更稳**

### 新功能

- 任务收尾增加证据链校验：待办未完成、无工具回执或文件不存在时不会误报「已完成」
- 痛点记忆：自动记录并检索常见踩坑（如 API Key、微信发送、路径问题）

### 改进

- 安装包内嵌 ppt-master 技能资源，启动时不再从 GitHub 额外下载（做 PPT 仍须大模型 API 联网）
- 启动与 `/api/health` 可查看 PPT 等内置 skill 是否就绪
- 发版流程自动清理 Gitee/GitHub 上过旧的 Release，保留最近 3 个版本
- 欢迎页 SciPilot 插件显示「科研数据可视化」，不再出现「未命名」

### 修复

- 覆盖安装/更新后，大模型服务商与视觉辅助开关尽量保持更新前选择
- 保存视觉/生图开关时同步写入 profile，避免下次启动被关
...
- 启动：内置 ppt-master 资源不再在启动阶段同步从 GitHub 下载，避免国内网络慢时 45 秒超时弹「后端未响应」
- 启动：skill 资源改为后台下载，失败仅记录日志，不阻塞进入主界面
...
- MCP 扩展：启动子进程时正确传入 PATH 与自定义环境变量，修复部分 MCP 服务无法连接
- 一键更新：下载链接仅允许官方 Gitee/GitHub Release 直链，SHA256 必须与 Release 附件一致

### 改进

- 全库 ruff lint 清理（160+ 项），提升打包与启动稳定性
- PPT 任务检测独立为 `ppt_task` 模块，减少做演示文稿时误用草稿工具
- 新增 `scripts/run-typecheck.cmd`，便于后续类型检查治理
...
- Agent 工具 `send_weixin_contact_message`：通过微信桌面版向指定联系人发送文字消息

### 修复

- 微信：助手声称「已发给某某」但对方未收到（无工具回执时剥掉假成功并提示核对）
- 微信：用户只要发文字给联系人时，回合末误把 `delivered/` 里历史 docx 连环推给当前对话
- 桌面：125% DPI 下侧栏被窄屏 CSS 误隐藏
- 桌面：最大化后还原需点两次标题栏按钮

### 改进

- 微信纯文字发联系人：跳过附件自动补发；系统提示禁止为此任务生成 docx/复制文件

---

## 1.4.3（2026-06-15，热补丁）
...
- 一键更新：robocopy 后校验 `Friday.exe` SHA256，文件被占用时回滚并提示
- 一键更新：强化安装目录进程退出，后端就绪即确认更新成功
- MCP：打包版解析内置 Node `npx`，找不到命令时仅告警一次
...
- 微信：每轮对话重复发送已推送过的 docx/pdf（按制品索引跳过已发文件）
- 微信桥接：一键部署后 `openclaw-weixin` 缺 `zod` 导致 Gateway 启动失败（自动补装运行时依赖）
- 微信长任务：API 响应超时误报「无法连接 API 服务器」（改为「API 响应超时」并提示稍后重试）
- 应用内更新：SHA256 校验失败时返回可读错误而非静默失败

### 改进

- 微信桥接插件升级 `openclaw-weixin@2.4.4`；已安装但缺依赖时自动 `npm install` 修复
- 更新源 `SHA256SUMS` 下载增加重试，弱网更稳
- 设置页更新相关错误提示与 release 哈希校验流程加固
