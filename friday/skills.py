...
_BUILTIN: list[dict[str, Any]] = [
    ...
    {"id": "file-downloads", "label": "整理下载文件夹", "icon": "📁", "category": "files",
     "prompt": "查看我的下载文件夹，按文件类型整理；先只读列出计划，等我确认后再移动，不要直接删除。"},
    {"id": "file-desktop", "label": "分析桌面", "icon": "🧹", "category": "files",
     "prompt": "查看桌面上的文件，分析是否杂乱，给出整理建议（先只读，不要移动或删除）。"},
    ...
    {"id": "doc-weekly", "label": "生成周报", "icon": "📝", "category": "docs",
     "prompt": "在我的文档目录生成一份本周工作周报 docx；同时按 ppt-master 工作流做一份配套汇报 pptx（先 read_text_file 读取内置 ppt-master 的 SKILL.md 并严格跟流程，禁止 create_pptx）。"},
    {"id": "doc-ppt", "label": "做汇报 PPT", "icon": "📊", "category": "docs",
     "prompt": "在我的文档目录制作一份项目汇报 pptx，共 5 页：封面、背景、进展、问题、下一步。请按内置 ppt-master 工作流执行（read_text_file 读 SKILL.md → 串行流水线 → svg_to_pptx 导出），禁止 create_pptx。"},
    ...
    {"id": "daily-dup", "label": "查重复文件", "icon": "🗂", "category": "daily",
     "prompt": "在我的文档和下载文件夹中查找重复文件，先列出结果；删除前须我逐项确认。"},
